...
public class TeacherMain {
    public static void main(String[] args) {
        ...
        //이상의 field의 element 를 출력하는 반복문을 참고하여 향상된 포문
        ...
        for (int i = 0 ; i < methods.length ; i++){
            System.out.println("필드명 출력 : " + methods[i].getName());
            System.out.println("리턴 타입 출력 : " + methods[i].getReturnType());
        }

        for (Method method : methods){
            System.out.println("필드명 출력 : " + method.getName());
            System.out.println("리턴 타입 출력 : " + method.getReturnType());
        }

        ch16_object.Teacher teacher3 = new ch16_object.Teacher("안근수", "코리아아이티");
